	...
	def decode_ins(self, only_labels = False):
		...
		if '#Radr' in ins_str:
			...
			if only_labels:
				...
				if radr_ in self.labels: ins_str = ins_str.replace('#Radr', self.labels[label[2]][0] + label[0])
...
''')
		if args.vector:
			if self.start < 6: self.printf('; Reset vectors')
			for k, v in self.rst_vct_names.items():
