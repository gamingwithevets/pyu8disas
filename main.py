...
class Disasm:
	...
	def decode_ins(self):
		...
		ins_len = 2
		num_ints_list = [sum(isinstance(_, int) for _ in ins[0]) for ins in self.instructions]
		candidates = []
		num_ints_c = []

		word, raw_bytes = self.read_ins()
		raw_bytes_int = int.from_bytes(raw_bytes, 'big')
		ins_str = f'D{"W" if ins_len == 2 else "D"} {self.format_hex_w(raw_bytes_int) if ins_len == 2 else self.format_hex_dd(raw_bytes_int)}'
		for j in range(len(self.instructions)):
			...
			if score >= score_cond and word[0] == ins[0][0]:
				...
				if len(ins) > 2: conditions.extend((
					...
					'#bit_offset' in ins[2] and int(f'{word[2]:04b}'[0]) != ins[0][2],
					))
				if len(ins) > 3: conditions.extend((
					ins[3] in ('#width', '#imm7') and int(f'{word[2]:04b}'[0]) != ins[0][2],
					'ER#1' in ins[3] and (word[2] & 1) != 0,
					'XR#1' in ins[3] and (word[2] & 2) != 0,
					'QR#1' in ins[3] and (word[2] & 3) != 0,
					'#Disp6' in ins[3] and int(f'{word[2]:04b}'[:2], 2) != ins[0][2],
					))
		...
		if len(candidates) > 0:
			...
			ins = self.instructions[candidates[num_ints_c.index(max(num_ints_c))]]
			ins_str = ins[1]
	...
	def disassemble(self, args):
		...
		tab = '\t'
		if args.addresses: format_ins = lambda addr, ins_op, ins_len, ins_str: f'{self.fmt_addr(addr)}\t{format(ins_op, "0"+str(ins_len*2)+"X")}\t{tab if ins_len < 3 else ""}{tab if ins_len < 5 else ""}{ins_str}'
