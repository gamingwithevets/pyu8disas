...
def decode_ins():
	...
	raw_bytes_int = int.from_bytes(raw_bytes)
	...
	if any(val in ins_str for val in vals):
		...
		ins_len += 2
		repl = lambda x: ins_str.replace(x, f'{format(word[1], "X")}:{format_hex_w(int.from_bytes(raw_bytes2))}')
	...
	if '#Cadr' in ins_str:
		_, raw_bytes2 = read_ins()
		ins_len += 2
		cadr = word[1] * 0x10000 + int.from_bytes(raw_bytes2)
		if cadr % 2 != 0: cadr -= 1
		if cadr > 5:
			skip = False
			for label in labels:
	...
	int_str = ins_str.replace('#bit_offset', word[2] & 0x7f) 
	...
	ins_str = ins_str.replace('#imm7', f'#{format_hex(imm7(comb_nibbs(word[2:])).value)}')
	ins_str = ins_str.replace('#width', f'#{format_hex(unsigned7(comb_nibbs(word[2:])).value)}')
...
def disassemble(interrupts: bool = True):
	...
	get_op = lambda addr, length = 2: int.from_bytes(conv_little(input_file[addr:addr+length]))
